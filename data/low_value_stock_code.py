...
class low_value_stock():
    ...
    def final_data_to_df(self, market_type):
        connection = None
        # try:
        connection = pymysql.connect(host='localhost',
                                     user='root',
        ...
        with connection.cursor() as cursor:
            ...
            data = pd.read_sql_query(sql, connection)[['kr_stock_code', 'market', 'kr_stock_name']]
            data = data.set_index('kr_stock_code')
            col = low_value_stock().value()
            if market_type == 'kospi':
                data = data.loc[list(set(col) - set(list(data[data['market'] == 'KOSDAQ'].index)))][['kr_stock_name']]
                data = data.reset_index()
                data.columns = ['2kr_stock_code', '2kr_stock_code']
                data = data.reset_index()
                data.columns = ['1num', '2kr_stock_code', '3kr_stock_name']
                data["1num"] = [i + 1 for i in data['1num']]
                data["2kr_stock_code"] = [str(i).zfill(6) for i in data['2kr_stock_code']]
                ...
                data = data.loc[list(set(col) - set(list(data[data['market'] == 'KOSPI'].index)))][['kr_stock_name']]
                data = data.reset_index()
                data.columns = ['2kr_stock_code', '2kr_stock_code']
                data = data.reset_index()
                data.columns = ['1num', '2kr_stock_code', '3kr_stock_name']
