...
from data.us_stock_daily import us_stock_daily
from data.kr_stock_weekly import kr_stock_weekly
from data.kr_stock_daily import kr_stock_daily
from data.us_stock_weekly import us_stock_weekly
...
# # 미국 주식 업데이트 코드 (하루 짜리)
# import pandas as pd
# data = pd.read_csv('us_stock_daily.csv')
# connection = pymysql.connect(host='localhost', user='root', password='0000', charset='utf8', db='web_db')
# cur = connection.cursor()
# for i in range(len(data)):
#     value = '(' + str(list(data.loc[i]))[1:-1] + ')'
#     sql = "INSERT INTO us_stock_daily (us_stock_code,date,us_stock_close, us_stock_close_diff, us_stock_open, us_stock_high, us_stock_low, us_stock_volume) VALUES {};".format(value)
#     cur.execute(sql)
#     connection.commit()
# connection.close()